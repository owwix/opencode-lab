/**
 * Normal Docker-isolated OpenCode launcher.
 *
 * The trusted host launcher resolves one canonical workspace, runs preflight,
 * enforces foreground ownership, selects optional service profiles,
 * materializes project-scoped configuration, creates launch identities and
 * capability leases, starts matching helpers/containers, and cleans up exact
 * launch registrations. Credentials remain in fixed-purpose processes and are
 * never copied into the project or OpenCode environment. Architecture:
...
try {
  ...
  console.error(
    `OpenCode Lab project setup is invalid: ${error instanceof Error ? error.message : String(error)}`
  );
  process.exit(1);
}
