...
test("lab entrypoint documents setup, workflow profile truth, and dry-run pruning", () => {
  const entry = read("scripts/opencode-entry.mjs");
  assert.match(entry, /opencode-lab prune \[--apply\]/u);
  assert.match(entry, /args\[0\] === "prune"/u);
  assert.match(
    entry,
    /quit and relaunch with \\`lab --with-research\\` or \\`lab --with-design\\`/u
  );
});
...
test("state init performs recursive ownership repair only for migrations", () => {
  ...
  assert.match(
    compose,
    /opencode-state-init:[\s\S]*image: opencode-lab-opencode:local/u
  );
  assert.match(
    compose,
