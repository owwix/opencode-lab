...
const qualityRoot = resolve(
  process.env.QUALITY_STATE_ROOT ?? join(harnessRoot, ".quality")
);
...
function writeHeartbeat(run, phase = null, status = "running") {
  ...
  const now = Date.now();
  atomicWriteJson(runPath(run.id, "heartbeat.json"), {
    runId: run.id,
...
async function withWorkerLease(initialRun, operation, action) {
  ...
  try {
    ...
    lease.stop(latest.state === "cancelled" ? "cancelled" : "finished");
  }
}
...
function refreshRun(run) {
  ...
  if (!run.commands.length) {
    run.commands = readPackageCommands(run.workspace, run.requirements, {
      release:
        run.releaseRequested ||
        run.requirements.security ||
        run.requirements.deployment
    });
  }
  return saveRun(run);
}
...
function cleanup(options) {
  ...
  if (!run.clean) {
    fail(
      "Worktree has uncommitted changes. Preserve or discard them explicitly before cleanup."
    );
  }
...
function metrics(options) {
  const result = summarizeRuns(listRuns(), {
...
--max-output-bytes, --max-tokens, --max-cost, and --max-tool-calls values.
